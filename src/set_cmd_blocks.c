...
void	create_add_new_block(t_list **cmd_blocks)
{
	t_list *new_elem;
	t_list *temp;
...
t_list *set_cmd_blocks(char **tokens, char **envplist)
{
	t_list	*cmd_blocks;
	t_list	*temp;
	int		i;
	...
	i = 0;
	pipes = 0;
	create_add_new_block(&cmd_blocks);
	temp = cmd_blocks;
	while (tokens[i] != '\0')
	{
		while (ft_strcmp(tokens[i], "|") != 0)
		{
